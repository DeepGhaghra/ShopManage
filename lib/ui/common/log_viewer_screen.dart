...
class _LogViewerScreenState extends ConsumerState<LogViewerScreen> {
  ...
  List<LogEntry> _getFilteredLogs(LogService logService) {
    List<LogEntry> logs = logService.logs.toList().reversed.toList(); // newest first
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          PopupMenuButton<String>(
              ...
              if (val == 'copy') {
                ...
                showDialog(
                  ...
                  builder: (ctx) => AlertDialog(
                    ...
                    content: const Text('This will remove all in-memory logs. File logs will remain on disk.'),
                    actions: [
            ...
            itemBuilder: (_) => [
              const PopupMenuItem(value: 'copy', child: Row(children: [Icon(Icons.copy, size: 18), SizedBox(width: 8), Text('Copy All Logs')])),
              const PopupMenuItem(value: 'clear', child: Row(children: [Icon(Icons.delete_outline, size: 18, color: Colors.red), SizedBox(width: 8), Text('Clear Logs', style: TextStyle(color: Colors.red))])),
            ],
          ),
        ],
      ),
          ...
          child: Column(
            children: [
              ...
              Container(
                ...
                child: Row(
                  children: [
                    _StatChip(label: 'All', count: logService.totalCount, color: AppColors.primary, isSelected: _filterLevel == null, onTap: () => setState(() => _filterLevel = null)),
                    const SizedBox(width: 8),
                    _StatChip(label: 'Success', count: logService.successCount, color: Colors.green, isSelected: _filterLevel == LogLevel.success, onTap: () => setState(() => _filterLevel = _filterLevel == LogLevel.success ? null : LogLevel.success)),
                    const SizedBox(width: 8),
                    _StatChip(label: 'Errors', count: logService.errorCount, color: Colors.red, isSelected: _filterLevel == LogLevel.error, onTap: () => setState(() => _filterLevel = _filterLevel == LogLevel.error ? null : LogLevel.error)),
                    const SizedBox(width: 8),
                    _StatChip(label: 'Warnings', count: logService.warningCount, color: Colors.orange, isSelected: _filterLevel == LogLevel.warning, onTap: () => setState(() => _filterLevel = _filterLevel == LogLevel.warning ? null : LogLevel.warning)),
                  ],
                ),
              ),
                            ...
                            child: InkWell(
                              ...
                              child: Padding(
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    Icon(_levelIcon(log.level), color: color, size: 20),
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        Icon(_levelIcon(log.level), color: _levelColor(log.level), size: 24),
                        const SizedBox(width: 8),
                        Text(log.level.name.toUpperCase(), style: TextStyle(fontWeight: FontWeight.w900, color: _levelColor(log.level))),
                        const Spacer(),
                        IconButton(
...
class _StatChip extends StatelessWidget {
  ...
  const _StatChip({required this.label, required this.count, required this.color, required this.isSelected, required this.onTap});
    ...
    return Expanded(
      child: GestureDetector(
        ...
        child: Container(
          ...
          child: Column(
            children: [
              Text('$count', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 16, color: isSelected ? color : AppColors.textPrimary)),
              Text(label, style: TextStyle(fontSize: 9, fontWeight: FontWeight.w600, color: isSelected ? color : Colors.grey.shade500)),
            ],
          ),
        ),
      ),

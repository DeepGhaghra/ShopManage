...
class _StockScreenState extends ConsumerState<StockScreen> {
  ...
  List<Map<String, dynamic>> _filterStock(
                      ...
                      SliverFillRemaining(
                        ...
                        child: EmptyStateView(
                          ...
                          message: _searchQuery.isEmpty
                              ? 'There is no stock available in this warehouse.'
                        ...
                        sliver: SliverList(
                          delegate: SliverChildBuilderDelegate((
                            ...
                            final design =
                                ...
                                {};
                            final location =
                                row['locations'] as Map<String, dynamic>? ?? {};
...
class _AddStockDialogState extends ConsumerState<_AddStockDialog> {
  ...
  void _showRenameLocationDialog(
    BuildContext context,
    WidgetRef ref,
    int locationId,
    String currentName,
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Add new stock to the selected shop',
                  ...
                  productHeadsAsync.when(
                    data: (heads) => _buildAutocomplete<ProductHead>(
                      ...
                      initialValue: null,
                      suggestions: (query) => heads.where(
                        (h) => h.productName.toLowerCase().contains(
                          query.toLowerCase(),
                        ),
                      ),
                      onSelected: (head) =>
                          setState(() => _selectedProductHead = head),
                      displayStringForOption: (head) => head.productName,
                      onClear: () =>
                          setState(() => _selectedProductHead = null),
                    ),
                  ...
                  locationsAsync.when(
                    data: (locations) =>
                        _buildAutocomplete<Map<String, dynamic>>(
                          label: 'Storage Location',
                          icon: Icons.location_on_rounded,
                          initialValue: _selectedLocationName,
                          suggestions: (query) => locations.where(
                            (l) => (l['name'] as String).toLowerCase().contains(
                              query.toLowerCase(),
                            ),
                          ),
    ...
    try {
      ...
      if (mounted) {
        ref
            ...
            .success(
              'Stock',
              'Added ${_quantity} units of "${_designController.text.trim()}" to "${_selectedLocationName ?? 'location'}"',
            );
        ref.invalidate(shopStockProvider);
        nav.pop();
      ...
      ref
          ...
          .error(
            'Stock',
            'Failed to add stock "${_designController.text.trim()}"',
            e,
            stack,
          );
      if (mounted) {
...
class _TransferStockDialog extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_TransferStockDialog> createState() =>
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  if (option is Map<String, dynamic>) {
                    ...
                    if (ph is Map)
                      phData = ph as Map<String, dynamic>;
                    else if (ph is List && ph.isNotEmpty)
                      phData = ph.first as Map<String, dynamic>;
                    ...
                    if (phData != null) {
                      ...
                      if (folder is Map)
                        folderData = folder as Map<String, dynamic>;
                      else if (folder is List && folder.isNotEmpty)
                        folderData = folder.first as Map<String, dynamic>;

                      final brand =
                          folderData?['folder_name']?.toString() ?? '';
  ...
  Future<void> _fetchAvailableLocations(int designId) async {
    ...
    try {
      ...
      final locations = await ref
          .read(stockRepositoryProvider)
          .getAvailableLocationsForDesign(activeShop.id, designId);
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Move inventory within the selected shop',
                    textAlign: TextAlign.center,
                    style: TextStyle(fontSize: 12, color: Colors.grey.shade600),
                  ),
                  const SizedBox(height: 28),
                  ...
                  stockAsync.when(
                    data: (stockItems) {
                      ...
                      for (var item in stockItems) {
                        final design =
                            item['products_design'] as Map<String, dynamic>?;
                        if (design != null) {
                      ...
                      availableDesigns.sort((a, b) {
                        final da = (a['design_no']?.toString() ?? '')
                            .toUpperCase();
                        final db = (b['design_no']?.toString() ?? '')
                            .toUpperCase();
                      ...
                      Container(
                        ...
                        child: Row(
                          children: [
                            ...
                            Expanded(
                              child: Text(
                                'No stock available for this design in any location.',
                          ...
                          children: [
                            ...
                            Wrap(
                              ...
                              children: _availableLocations.map((loc) {
                                ...
                                final isSelected =
                                    _fromLocationId == locData['id'];
                                return ChoiceChip(
                                  label: Text(
                                    '${locData['name']} (${loc['quantity']})',
                                  ),
                                  selected: isSelected,
                                  ...
                                  onSelected: (selected) {
                                    if (selected) {
                                      setState(() {
                                        ...
                                        _maxAvailableQty =
                                            loc['quantity'] as int;
                                        if (_quantity > _maxAvailableQty)
                                          _quantity = _maxAvailableQty;
                  ...
                  if (_fromLocationId != null) ...[
                    ...
                    TextFormField(
                      ...
                      validator: (val) {
                        final n = int.tryParse(val ?? '');
                        if (n == null || n < 1) return 'Enter a valid quantity';
                        if (n > _maxAvailableQty)
                          return 'Cannot transfer more than $_maxAvailableQty';
                        return null;
                      },
                    ),
                    const SizedBox(height: 28),
    ...
    try {
      ...
      if (mounted) {
        ref
            ...
            .success(
              'Stock',
              'Transferred ${_quantity} units of design #${_selectedDesign!['design_no']}',
            );
        ref.invalidate(shopStockProvider);
        nav.pop();
        scaffoldMsg.showSnackBar(
      ...
      ref
          ...
          .error(
            'Stock',
            'Stock transfer failed for design #${_selectedDesign?['design_no'] ?? 'unknown'}',
            e,
            stack,
          );
      if (mounted) {
